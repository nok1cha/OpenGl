...

{
	...
	if (turn == 1) DrawString(GLUT_BITMAP_HELVETICA_18, "Player1's turn", 50, 30);

	else DrawString(GLUT_BITMAP_HELVETICA_18, "Player2's turn", 50, 30); //moved point for time display
	...
	case 11: if (turn == 1) turn = 2; else turn = 1; // over turn if not finished in limit seconds
